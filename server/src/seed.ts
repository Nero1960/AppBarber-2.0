...
const seed = async () => {
    try {
        ...
        console.log(colors.bgGreen.white('Database connection established successfully.'));

        await db.sync();
        await db.sync({ force: true });
        console.log(colors.bgGreen.white('Tables dropped and recreated.'));
        ...
        console.log(colors.bgGreen.white(`Seeded ${users.length} users.`));
        ...
        const barbers = await Barber.bulkCreate(barbersData);
        console.log(colors.bgGreen.white(`Seeded ${barbers.length} barbers.`));
        ...
        console.log(colors.bgGreen.white(`Seeded ${services.length} services.`));
        ...
        const appointmentsData = [
            ...
            { user: users[3], barber: barbers[2], date: '2026-08-17', time: '10:00:00', status: 'confirmed', services: [5, 3] },
            { user: users[4], barber: barbers[0], date: '2026-08-17', time: '11:30:00', status: 'pending', services: [0] },
            { user: users[5], barber: barbers[4], date: '2026-08-18', time: '14:00:00', status: 'confirmed', services: [6] },
            { user: users[6], barber: barbers[1], date: '2026-08-18', time: '16:30:00', status: 'pending', services: [2] },
            { user: users[7], barber: barbers[3], date: '2026-08-19', time: '09:00:00', status: 'confirmed', services: [0, 4, 7] },
        ];
        ...
        console.log(colors.bgGreen.white(`Seeded ${appointmentsData.length} appointments.`));
        ...
        console.log(colors.bgGreen.white(`Seeded ${testimonialsData.length} testimonials.`));

        await db.close();
        console.log(colors.bgGreen.white('Seed completed successfully.'));
    } catch (error) {
        console.log(colors.bgRed.white('Seed failed:'), error);
        process.exit(1);
    }
};
