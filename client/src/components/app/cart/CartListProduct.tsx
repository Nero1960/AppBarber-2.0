...
import { decrementCartQuantity, incrementCartQuantity, removeFromCart } from "@/api/CartApi"
                            ...
                            {listCarts.map((item) => (
                                <Fragment key={item.cartId}>
                                    {item.product.map(products => (
                                        <TableRow key={products.productId} className="border-brown-400 hover:bg-brown-500 duration-300">
                                            <TableCell>
                                                <div className="flex items-center space-x-4">
                                                    <Image src={`${import.meta.env.VITE_IMAGE_URL}/${products.image}`} width={50} height={50} className="rounded-md" />
                                                    <div>
                                                        <h3 className="font-semibold text-white-500">{products.name}</h3>
                                                        <p className="text-xs text-brown-200">{products.description}</p>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <div className="flex items-center space-x-2">
                                                    <Button
                                                        ...
                                                        size="icon"
                                                        onClick={() => handleClickDecrement(products.CartDetails.cartId, products.CartDetails.productId)}
                                                    >
                                                    ...
                                                    >{products.CartDetails.quantity}</Button>
                                                    <Button
                                                        variant="outline"
                                                        className="bg-transparent border-none hover:bg-transparent"
                                                        size="icon"
                                                        onClick={() => handleClickIncrement(products.CartDetails.cartId, products.CartDetails.productId)}
                                                    >
                                                        <PlusCircle className="h-4 w-4 text-Primary-500" />
                                                    </Button>
                                                </div>
                                            </TableCell>
                                            <TableCell className="text-white-500">{formatToCordobas(products.CartDetails.unit_price)}</TableCell>
                                            <TableCell className="text-Primary-500">{formatToCordobas(products.CartDetails.subtotal)}</TableCell>
                                            <TableCell>
                                                <Button
                                                    variant="ghost"
                                                    className="hover:bg-brown-500"
                                                    size="icon"
                                                    onClick={() => handleClickRemoveItem(products.CartDetails.cartId, products.CartDetails.productId)}
                                                >
                    ...
                    <Card className="bg-brown-500 border-none w-full">
                                ...
                                {listCarts.map((item) => (
                                    <Fragment key={item.cartId}>
                                        ...
                                        {item.product.map(productItem => (
                                            <div key={productItem.productId} className="flex justify-between text-sm">
                                                <span className="text-white-500">{productItem.name} (x{productItem.CartDetails.quantity})</span>
                                                <span className="text-Primary-500">{formatToCordobas(productItem.CartDetails.unit_price * productItem.CartDetails.quantity)}</span>
                                            </div>
                                        ))}
                        ...
                        <CardFooter className="flex flex-col space-y-4">
                            <div className="flex justify-between items-center w-full text-lg font-semibold">
                                ...
                                <span className="text-Primary-500 font-bold">{formatToCordobas(listCarts.reduce((acc, cur) => acc + cur.product.reduce((acc2, cur2) => acc2 + cur2.CartDetails.subtotal, 0), 0))}</span>
                            </div>
                            <Button
                                disabled={
                                    listCarts.reduce((acc, cur) => acc + cur.product.reduce((acc2, cur2) => acc2 + cur2.CartDetails.quantity, 0), 0) === 0
                                }
